...
class SE2PathFollowDWA:
    ...
    def __init__(
        ...
        Args:
            env (DiffDriveSE2PathFollow): The environment.
        ...
        """
        assert isinstance(env, DiffDriveSE2PathFollow), 'env must be a DiffDriveSE2PathFollow'
    ...
    def _sample_action_seq(self):
        for lin_vel in np.arange(self._min_lin_vel, self._max_lin_vel + 1e-6, self._lin_vel_steps):
            for ang_vel in np.arange(self._min_ang_vel, self._max_ang_vel + 1e-6, self._ang_vel_steps):
                # Skip zero velocity
    ...
    def run(self, state: np.ndarray) -> np.ndarray:
        ...
        for lin_vel, ang_vel in self.sampled_action_seq:
            ...
            total_cost = self.env.traj_cost_func(traj, action)
