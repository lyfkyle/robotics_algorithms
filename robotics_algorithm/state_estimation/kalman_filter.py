...
class KalmanFilter:
    def __init__(self, env: BaseEnv):
        ...
        self.A, self.B = env.state_transition_jacobian(
            None, None
        )  # As state transition is linear, A and B are constant
        self.R = env.state_transition_covariance_matrix

        # measurement function z = Hx + delta
        self.H = env.observation_jacobian(None, None)  # As observation is linear, H are constant
