...
class TwoDLocalizationWithFeature(object):
    ...
    def __init__(self, size=10, initial_state=[5, 5, 0], control_noise=[0.1, 0.1], meas_noise=[0.1, 0.1, 1e-10]):
        ...
        self.Q = np.array([[meas_noise[0], 0, 0], [0, meas_noise[1], 0], [0, 0, meas_noise[2]]], dtype=np.float32)
    ...
    def measure(self):
        ...
        for i in range(self.num_of_features):
            ...
            # add random noise
            meas_feature += np.array([randn() * self.meas_noise[0], randn() * self.meas_noise[1], 0])
    ...
    def state_transition(self, state, control):
        ...
        if true_ang_vel != 0:
            ...
            new_state = state + np.array(
                [
                    -temp * math.sin(theta) + temp * math.sin(theta + true_ang_vel * self.dt),
            ...
            new_state = state + np.array(
                [true_vel * math.cos(theta) * self.dt, true_vel * math.sin(theta) * self.dt, 0]
            )
    ...
    def measurement_func_each_feature(self, state, feature_idx):
        ...
        if self.meas_correspondence:
            meas = np.array(
                [
                    math.sqrt((feature["pos"][0] - state[0]) ** 2 + (feature["pos"][1] - state[1]) ** 2),
    ...
    def measuremnt_prob_each_feature(self, state, meas_feature):
        ...
        prob = math.e ** -(error_1**2 / (2 * math.sqrt(self.meas_noise[0]))) * math.e ** -(
            error_2**2 / (2 * math.sqrt(self.meas_noise[1]))
        )  # ignore prob of ind. Assume ind is always correct
        # print(prob)
        return prob
    ...
    def control_jacobian(self, state, control):
        ...
        self.A = np.array(
            [
                [1, 0, -temp * math.cos(theta) + temp * math.sin(theta + ang_vel * self.dt)],
    ...
    def process_noise_jacobian(self, state, control):
        ...
        self.V = np.array(
            [
                [
                    (-math.sin(theta) + math.sin(theta + ang_vel * self.dt)) / ang_vel,
                    vel * (math.sin(theta) - math.sin(theta + ang_vel * self.dt)) / (ang_vel**2)
                    + vel * math.cos(theta + ang_vel * self.dt) * self.dt / ang_vel,
                ],
                [
                    (math.cos(theta) - math.cos(theta + ang_vel * self.dt)) / ang_vel,
                    -vel * (math.cos(theta) - math.cos(theta + ang_vel * self.dt)) / (ang_vel**2)
                    + vel * math.sin(theta + ang_vel * self.dt) * self.dt / ang_vel,
                ],
                [0, self.dt],
            ]
        )
    ...
    def meas_jacobian(self, state, meas):
        ...
        for feature in self.features:
            ...
            H = np.array(
                [
                    [
                        -(feature["pos"][0] - state[0]) / math.sqrt(temp),
                        -(feature["pos"][1] - state[1]) / math.sqrt(temp),
                        0,
                    ],
