...
class MPPIController:
    ...
    def calc_control_input(self, state: np.ndarray) -> Tuple[float, np.ndarray]:
...
def run_simulation_mppi_pathtracking() -> None:
    ...
    print(
        f"[INFO] delta_t : {delta_t:.2f}[s] , sim_steps : {sim_steps}[steps], total_sim_time : {delta_t*sim_steps:.2f}[s]"
    )
    ...
    for i in range(sim_steps):
        ...
        print(
            f"Time: {i*delta_t:>2.2f}[s], x={current_state[0]:>+3.3f}[m], y={current_state[1]:>+3.3f}[m], yaw={current_state[2]:>+3.3f}[rad], v={current_state[3]:>+3.3f}[m/s], steer={optimal_input[0]:>+6.2f}[rad], accel={optimal_input[1]:>+6.2f}[m/s]"
        )
    ...
    # save animation
    vehicle.save_animation(
        "mppi_pathtracking_demo.mp4", interval=int(delta_t * 1000), movie_writer="ffmpeg"
    )  # ffmpeg is required to write mp4 file
