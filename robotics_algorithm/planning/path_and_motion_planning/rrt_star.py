...
class RRTStar(object):
    ...
    def __init__(self, env, sample_func, vertex_expand_func, edge_col_check_func, distance_func, num_of_samples):
        self.env = env
    ...
    def extend(self, v_target: tuple):
        ...
        nearest_neighbors = self.get_nearest_neighbors(np.array(all_nodes), np.array(v_target).reshape(1, 2))
        ...
        if v_new != v_cur:
            raw_neighbors = self.get_nearest_neighbors(
                np.array(all_nodes), np.array(v_new).reshape(1, 2), n_neighbors=10
            )
